...
function ComputerPage(props) {
    ...
    const [computerDetail, setComputerDetail] = useState({ ...sampleKeyValue });
    ...
    const [updateComputer, setUpdateComputer] = useState({ ...sampleKeyValue });
    ...
    const requestComputerList = async () => {
        try {
            const rs = await axios.get("http://localhost:8080/api/v1/computer/computers", {
                params
                // params : 객체데이터
                // computers?company=${params.company}&cpu=${params.cpu}
            });
            console.log(rs.data);
            setComputerList(rs.data);
        } catch (e) {
            console.error(e);
        }
    ...
    const handleSelectComputerClick = async (computerId) => {
        ...
        if (!data) {
            setComputerDetail({ ...sampleKeyValue });
            return
        }
    ...
    const closeModal = () => {
        setModalOpen(false);
        setUpdateComputer({ ...sampleKeyValue });
    }
    const handleUpdateComputerClick = async (computerId) => {
        setModalOpen(true);
        const responseData = await requestGetComputer(computerId);
    ...
    const requestUpdateComputer = async (computerId) => {
        let responseData = null
        try {
    ...
    const handleSearchClick = () => {
        ...
        setParams({ company: "", cpu: "" });

    }
    const handleRegisterSubmitClick = async () => {

        try {
            const rs = await axios.post("http://localhost:8080/api/v1/computer/register", registerComputer)
            if (rs.status === 200) {
                alert("등록성공 !");
                requestComputerList();
            }
    ...
    return (
        <div>
            ...
            <div css={layout}>
                <h2>목록</h2>
                <p>
                    <input type="text" name='company' onChange={handleSearchInput} value={params.company} placeholder='제조사' />
                    <input type="text" name='cpu' onChange={handleSearchInput} value={params.cpu} placeholder='CPU' />
