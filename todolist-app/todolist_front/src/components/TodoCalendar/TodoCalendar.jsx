...
function TodoMonthGroup({ month, dateOfCalendarData }) {
    const entriresOfDate = Object.entries(dateOfCalendarData);
    return <>
        <h2 css={s.monthTtile}>{month}월</h2>
        <div>
            {
                entriresOfDate.map(([date, todos]) => <TodoDateGroup key={month + date} date={date} todos={todos} />)
            }
        </div>
    </>
}
function TodoDateGroup({ date, todos }) {
    return <>
        ...
        <div>
            {
                todos.map(todo => <TodoBox todo={todo} />)
            }
        </div>
    </>
}
